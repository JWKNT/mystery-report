...
DATA = ROOT.parent.parent / "data" / "v3"
RAW = DATA / "raw_agents"
AUDIT = DATA / "audit"
...
def main() -> int:
    ...
    if method.get("lower_bound_z") != 1.645:
        errors.append("consensus ranking is not using the one-sided 95% lower bound")
    ...
    if collected_selections != 30_000:
        errors.append(f"expected exactly 30000 collected placements, found {collected_selections}")
    ...
    if len(selection_groups) != 300 or any(length < 0 or length > 100 for length in group_lengths):
