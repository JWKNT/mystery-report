...
def aggregate(observations: list[Observation], exclusions_path: Path) -> dict[str, Any]:
    ...
    for (agent_number, cluster_index), variants in grouped.items():
        ...
        for axis, rank in combined_ranks.items():
            ...
            raw_selections.append({
                ...
                "axis": axis,
                "rank": rank,
                "raw_title": chosen.identity.title,
                "raw_creator": chosen.identity.creator,
                "raw_year": chosen.identity.year,
                "raw_medium": chosen.identity.medium,
    ...
    for cluster_index in included_clusters:
        ...
        confidence_penalty = LOWER_BOUND_Z * global_composite_sd / math.sqrt(support + PRIOR_STRENGTH)
    ...
    return {
        "method": {
            ...
            "ranking_score": "one-standard-error lower confidence bound of the rank-adjusted Bayesian weighted mean",
        },
...
def main() -> int:
    ...
    parser.add_argument("--exclusions", type=Path, default=DATA_DIR / "audit" / "exclusions.json")
    parser.add_argument("--output-dir", type=Path, default=DATA_DIR / "audit")
