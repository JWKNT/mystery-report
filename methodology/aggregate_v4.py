#!/usr/bin/env python3
"""Reconcile and aggregate the v4 scored mystery consensus responses."""
...
def identity_from_work(work: dict[str, Any]) -> Identity:
    ...
    # Verified aliases and publication-metadata corrections that cannot be
    # inferred from punctuation alone. Parent/component decisions live in the
    # separately audited series-rollup configuration.
    if title in {"southern reach trilogy", "southern reach series"} and creator == "jeff vandermeer" and identity.medium in {"novel", "book_series"}:
    ...
    if title == "new york trilogy" and creator == "paul auster" and identity.medium in {"novel", "novella", "book_series"}:
    ...
    if title == "marble hornets" and creator in {"troy wagner and joseph delage", "troy wagner"}:
        return Identity("Marble Hornets", "", "Troy Wagner and Joseph DeLage", 2009, "web_serial")
    if title == "sins of father knox" and creator == "josef skvorecky":
        return Identity("Sins for Father Knox", identity.original_title, "Josef Škvorecký", 1973, "novel")
    if title == "mind forever voyaging" and creator == "steve meretzky":
        return Identity("A Mind Forever Voyaging", "", "Steve Meretzky", 1985, "video_game")
    if title == "monument mythos" and creator == "alex casanas":
        return Identity("The Monument Mythos", "", "Alex Casanas", 2020, "web_serial")
    ...
    if title == "danganronpa trigger happy havoc" and creator == "kazutaka kodaka":
        return Identity("Danganronpa: Trigger Happy Havoc", "", "Kazutaka Kodaka", 2010, "visual_novel")
    if title == "zero escape" and creator == "kotaro uchikoshi":
    ...
    if title in {"death among the undying", "death among the undead"}:
        return Identity("Death Among the Undead", "Shijin-sō no Satsujin", "Masahiro Imamura", 2017, "novel")
    if title == "case of the three detectives" and creator == "leo bruce":
        return Identity("Case for Three Detectives", "", "Leo Bruce", 1936, "novel")
    return identity
...
def apply_series_rollups(
    observations: list[Observation], config_path: Path,
) -> list[dict[str, Any]]:
    """Apply audited parent/component rules before ordinary identity clustering.

    Continuous components always roll into their configured parent. A
    self-contained component remains independent only when at least the
    configured number of agents nominated that component or one of its title
    aliases. Parent aliases always normalize to the parent. Media and creator
    constraints prevent adaptations from being combined across forms.
    """
    if not config_path.exists():
        return []
    config = json.loads(config_path.read_text())
    threshold = int(config.get("preserve_component_support", 10))
    decisions: list[dict[str, Any]] = []

    for rule in config.get("series", []):
        parent = Identity(**rule["parent"])
        creators = {normalize(value) for value in rule.get("creator_aliases", [parent.creator])}
        media = set(rule.get("media", [parent.medium]))

        def eligible(observation: Observation) -> bool:
            return observation.identity.medium in media and normalize(observation.identity.creator) in creators

        parent_titles = {normalized_title(value) for value in rule.get("parent_titles", [parent.title])}
        continuous_groups = [
            {normalized_title(value) for value in group["titles"]}
            for group in rule.get("continuous_components", [])
        ]
        standalone_groups = [
            {normalized_title(value) for value in group["titles"]}
            for group in rule.get("standalone_components", [])
        ]

        def matched_titles(observation: Observation) -> set[str]:
            return title_candidates(observation.identity)

        standalone_support = [
            len({
                observation.agent_number
                for observation in observations
                if eligible(observation) and matched_titles(observation) & titles
            })
            for titles in standalone_groups
        ]

        grouped_decisions: dict[tuple[Identity, str, int], set[int]] = defaultdict(set)
        observation_counts: Counter[tuple[Identity, str, int]] = Counter()
        for observation in observations:
            if not eligible(observation):
                continue
            titles = matched_titles(observation)
            reason = ""
            component_support = 0
            if titles & parent_titles:
                reason = "parent_alias"
            elif any(titles & group for group in continuous_groups):
                reason = "continuous_component"
            else:
                for index, group in enumerate(standalone_groups):
                    if titles & group:
                        component_support = standalone_support[index]
                        if component_support < threshold:
                            reason = "low_support_standalone_component"
                        else:
                            grouped_decisions[(observation.identity, "preserved_standalone_component", component_support)].add(
                                observation.agent_number
                            )
                            observation_counts[(observation.identity, "preserved_standalone_component", component_support)] += 1
                        break
            if not reason:
                continue
            original = observation.identity
            grouped_decisions[(original, reason, component_support)].add(observation.agent_number)
            observation_counts[(original, reason, component_support)] += 1
            observation.identity = parent

        for (identity, reason, component_support), agents in grouped_decisions.items():
            decisions.append({
                "from": asdict(identity),
                "to": asdict(parent) if reason != "preserved_standalone_component" else asdict(identity),
                "decision": reason,
                "component_agent_support": component_support,
                "agents": len(agents),
                "observations": observation_counts[(identity, reason, component_support)],
            })
    return decisions
...
def rank_adjustment(axis: str, ranks: dict[str, int]) -> float:
    ...
    work scored through another list but absent from this list is treated as
    the conservatively censored rank 101. Influence and traditionality receive
    no rank correction; traditional_mystery placement measures all-around
    achievement rather than traditionality purity.
    """
    if axis not in RANK_ADJUSTMENT_AXES:
        return 0.0
...
def aggregate(
    observations: list[Observation], exclusions_path: Path, series_rollups_path: Path,
) -> dict[str, Any]:
    if not observations:
        raise ValueError("No valid work observations")
    series_rollup_log = apply_series_rollups(observations, series_rollups_path)
    ...
    for (agent_number, cluster_index), variants in grouped.items():
        ...
        # Preserve the literal source placements before giving the retained
        # agent-work observation the union of ranks from reconciled aliases.
        # Otherwise mutating ``chosen.ranks`` can make its raw export repeat a
        # placement that belonged only to another alias in ``variants``.
    ...
    for cluster_index in included_clusters:
        for observation in unique_observations[cluster_index]:
            retained_observations.append({
                ...
                "source": asdict(observation.raw_identity),
    ...
    return {
        "method": {
            ...
            "rank_adjustment_axes": list(RANK_ADJUSTMENT_AXES),
            "confidence_support_denominator": "sqrt(actual agent support)",
            "placement_role": "ambition, fairness, and originality supply bounded ±5 rank corrections; traditional_mystery placement supplies nominations and support only; influence is scored only",
        ...
        "merge_log": merge_log,
        "series_rollup_log": series_rollup_log,
...
def main() -> int:
    ...
    parser.add_argument("--series-rollups", type=Path, default=BASE_DIR / "series_rollups.json")
    ...
    result = aggregate(observations, args.exclusions, args.series_rollups)
    ...
    (args.output_dir / "series-rollups.json").write_text(json.dumps(result["series_rollup_log"], ensure_ascii=False, indent=2) + "\n")
    ...
    summary = {
        ...
        "series_rollup_decisions": len(result["series_rollup_log"]),
